...
function initBookingModal() {
    // Deprecated in favor of Calendly
    // Keeping function to prevent errors if called
    const bookingButtons = document.querySelectorAll('[data-modal="booking"]');
    if (bookingButtons.length > 0) {
        // Legacy support if any buttons remain
        const modal = document.getElementById('booking-modal');
        bookingButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                if (typeof Calendly !== 'undefined') {
                    Calendly.initPopupWidget({ url: 'https://calendly.com/beydalyuman' });
                } else {
...
function setupInquiryForm(form) {
    ...
    // Handle course pre-fill from buttons
    document.querySelectorAll('a[data-course]').forEach(btn => {
        btn.addEventListener('click', function () {
            const courseText = this.getAttribute('data-course');
            const select = form.querySelector('#course-select');
            if (select) {
                Array.from(select.options).forEach(opt => {
                    if (opt.text.includes(courseText) || courseText.includes(opt.text)) {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (validateForm(form)) {
            ...
            // Collect data manually since name attributes might be missing
            const formData = {
                course: form.querySelector('#course-select')?.value || '',
                name: form.querySelector('#inq-name')?.value || '',
                phone: form.querySelector('#inq-phone')?.value || '',
                email: form.querySelector('#inq-email')?.value || '',
                experience: form.querySelector('#experience')?.value || '',
                goal: form.querySelector('#goal')?.value || '',
                notes: form.querySelector('#inq-notes')?.value || '',
                privacy: form.querySelector('#inq-privacy')?.checked || false,
                submittedAt: new Date().toISOString()
            };

            // Post to n8n Webhook
            // TODO: Replace with actual n8n webhook URL
            const WEBHOOK_URL = 'https://n8n.webhook.placeholder/beauty-academy-inquiry';
            ...
            fetch(WEBHOOK_URL, {
                ...
                .then(response => {
                    if (response.ok) {
                        closeModal(document.getElementById('booking-modal')); // Just in case
                        openModal(document.getElementById('success-modal'));
                        form.reset();
                    } else {
